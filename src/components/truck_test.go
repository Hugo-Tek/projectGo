...
func TestCreateTruckInvalidX(t *testing.T) {
	...
	if len(result) != len(trucks) {
		t.Errorf("Expected result to have length %d, got %d", len(trucks), len(result))
	}
}
...
func TestCreateTruckValidInputValues(t *testing.T) {
	...
	trucks := []Truck{}
	result := CreateTruck(parts, trucks)
	if result[0].Name != parts[0] || result[0].X != 10 || result[0].Y != 20 || result[0].MaxLoad != 30 || result[0].DayGone != 40 || result[0].IsGone != 0 || result[0].Status != "WAITING" || result[0].CurrentLoad != 0 {
		t.Errorf("Expected truck fields to be {%s, %d, %d, %d, %d, %s}, got {%s, %d, %d, %d, %d, %s}", parts[0], 10, 20, 30, 40, "WAITING", result[0].Name, result[0].X, result[0].Y, result[0].MaxLoad, result[0].DayGone, result[0].Status)
	}
}
