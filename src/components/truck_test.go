...
func TestCreateTruckValidInputValues(t *testing.T) {
	...
	trucks := []Truck{}
	result := CreateTruck(parts, trucks)
	if result[0].Name != parts[0] || result[0].X != 10 || result[0].Y != 20 || result[0].MaxLoad != 30 || result[0].Availability != 40 {
		t.Errorf("Expected truck fields to be {%s, %d, %d, %d, %d}, got {%s, %d, %d, %d, %d}", parts[0], 10, 20, 30, 40, result[0].Name, result[0].X, result[0].Y, result[0].MaxLoad, result[0].Availability)
	}
}
